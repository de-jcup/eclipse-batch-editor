...
import de.jcup.batcheditor.SimpleWordCodeCompletion;
import de.jcup.batcheditor.SimpleWordListBuilder;
import de.jcup.batcheditor.WordListBuilder;
...
public class BatchEditorSimpleWordContentAssistProcessor implements IContentAssistProcessor, ICompletionListener {
	...
	private class SimpleWordProposal implements ICompletionProposal, ICompletionProposalExtension7 {
		...
		public StyledString getStyledDisplayString(IDocument document, int offset,
			...
			try {
				...
				BatchEditorUtil.logError("Not able to set styles for proposal:" + word, e);
			}
			return styledString;
		}
	...
	public void assistSessionStarted(ContentAssistEvent event) {
		...
		BatchEditorPreferences preferences = BatchEditorPreferences.getInstance();
	...
	protected void addAllBatchKeyWords() {
		for (DocumentKeyWord keyword : BatchExternalKeyWords.values()) {
			addKeyWord(keyword);
		}
		for (DocumentKeyWord keyword : BatchBuildInKeywords.values()) {
			addKeyWord(keyword);
		}
		for (DocumentKeyWord keyword : BatchSpecialVariableKeyWords.values()) {
			addKeyWord(keyword);
		}
	}
