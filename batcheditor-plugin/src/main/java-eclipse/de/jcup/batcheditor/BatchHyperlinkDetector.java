...
public class BatchHyperlinkDetector extends AbstractHyperlinkDetector {
	...
	BatchHyperlinkDetector(IAdaptable editor) {
	...
	public IHyperlink[] detectHyperlinks(ITextViewer textViewer, IRegion region, boolean canShowMultipleHyperlinks) {
		...
		BatchEditor editor = adaptable.getAdapter(BatchEditor.class);
		if (editor == null) {
			return null;
		}
		...
		BatchLabel function = editor.findBatchLabel(functionName);
		if (function == null) {
			return null;
		}
		Region targetRegion = new Region(offsetLeft, functionName.length());
		return new IHyperlink[] { new BatchLabelHyperlink(targetRegion, function, editor) };
	}
