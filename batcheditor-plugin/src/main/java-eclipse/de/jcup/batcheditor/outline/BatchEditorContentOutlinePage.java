...
public class BatchEditorContentOutlinePage extends ContentOutlinePage implements IDoubleClickListener {
	private static ImageDescriptor IMG_DESC_LINKED = EclipseUtil.createImageDescriptor("/icons/outline/synced.png",
			BatchEditorActivator.PLUGIN_ID);
	private static ImageDescriptor IMG_DESC_NOT_LINKED = EclipseUtil
			.createImageDescriptor("/icons/outline/sync_broken.png", BatchEditorActivator.PLUGIN_ID);

	private BatchEditorTreeContentProvider contentProvider;
	private Object input;
	private BatchEditor editor;
	private BatchEditorOutlineLabelProvider labelProvider;
	...
	public BatchEditorContentOutlinePage(BatchEditor editor) {
		this.editor = editor;
		this.contentProvider = new BatchEditorTreeContentProvider();
	}

	public BatchEditorTreeContentProvider getContentProvider() {
	...
	public void onEditorCaretMoved(int caretOffset) {
		...
		if (contentProvider instanceof BatchEditorTreeContentProvider) {
			BatchEditorTreeContentProvider gcp = (BatchEditorTreeContentProvider) contentProvider;
